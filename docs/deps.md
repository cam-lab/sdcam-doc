...
!!! info "**ЗАМЕЧАНИЕ**"
    ...
    В случае использования виртуальных окружений необходимо при сборке явно указать директории с заголовочными файлами, в противном случае `libboost_numpy` не собирается: 

    ```shell
    ./b2 --with-python \
        include="/usr/include/python3.12" \
        include="$(python3.12 -c 'import numpy; print(numpy.get_include())')" \
        stage
    ```    

    Указывать нужно обе директории&nbsp;– если указать только вторую, то сборка завершится с ошибками, т.к. основные заголовки не будут найдены.
