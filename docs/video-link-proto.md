...
<div class="before-wavedrom-paragraph">
...
| `VST`  | Video Sync Tag. Признак синхронизации кадра или его элемента |
...
Если <code>CFT</code> активен, то остальные биты слова интерпретируются как показано ниже:
<p></p>
...
Если `CFG` неактивен, то следующим управляющим тегом является `VST`, который индицирует блок синхронизации &nbsp;– MDB (Meta Data Block). MDB содержит обязательную и необязательную (опциональную) части.
...
`MDBS` указывает длину блока, не включая слов с тегом, т.е. может иметь величину от \(0\) до \(2^{13} = 8192\) слова.
...
!!! warning "**ВАЖНОЕ ЗАМЕЧАНИЕ**"
    ...
    Протокол никак не регламентирует содержимое таких блоков, он только учитывает их длину в MDB. Формирование, разбор MDB и правильность информации в них лежат на протоколах более высоких уровней.


### Передача кадра

В общем случае передача начинается с переключения канала при помощи использования тега `CFG`. За ним следует кадровый блок метаинформации, обозначающий начало передачи кадра и содержащий служебную необходимою информацию о кадре, которая может использоваться на приёмной стороне. К обязательной части MDB относится номер кадра, временной штамп, размерность кадра и разрядность пиксела. 
...
wavedrom (
    {reg: [
        ...
        {"bits": 8, "name": "Frame Number[0]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},

        {"bits": 8, "name": "Frame Number[1]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},

        {"bits": 8, "name": "Frame Number[2]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},

        {"bits": 8, "name": "Frame Number[3]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},

        {"bits": 8, "name": "Timestamp[0]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},

        {"bits": 8, "name": "Timestamp[1]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},

        {"bits": 8, "name": "Timestamp[2]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},

        {"bits": 8, "name": "Timestamp[3]", "type": 7, "attr": ""},
        {"bits": 6, "name": "x",   "type": 1, "attr": ""},
        {"bits": 1,  "name": "VST: 0",  "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",  "type": 5, "attr": ""},
...
wavedrom (
    {reg: [
        ...
        {"bits": 14, "name": "Pixel 4", "type": 7, "attr": ""},
        {"bits": 1,  "name": "VST: 0",      "type": 6, "attr": ""},
        {"bits": 1,  "name": "CFT: 0",      "type": 5, "attr": ""},
...
При приёме алгоритм следующий:
  ...
  1. производится разбор строчного MDB&nbsp;– фиксируется номер строки. Строки внутри при передаче не обязаны следовать строго одна за другой[^1], но все они должны быть без ошибок приняты до прихода следующего кадра. В противном случае кадр считается испорченным, все принятые данные отбрасываются и приём начинается с п.1;
  ...
  [^1]: Существуют форматы кадров, которые формируют последовательность строк не строго по порядку&nbsp;– например, при чересстрочной развёртке.